...
namespace Decorator
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            foreach(RadioButton rdo in grpStorage.Controls.OfType<RadioButton>())
            {
                rdo.CheckedChanged += new EventHandler(options_CheckedChanged);
            }

            foreach (RadioButton rdo in grpMemory.Controls.OfType<RadioButton>())
            {
                rdo.CheckedChanged += new EventHandler(options_CheckedChanged);
            }

            chkApplyDiscount.CheckedChanged += new EventHandler(options_CheckedChanged);
